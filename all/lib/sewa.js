...
const expiredCheck = (pasa, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            pasa.sendMessage(_dir[position].id, { text: `Masa sewa di grup ini telah habis, bot otomatis keluar!` })
            .then( res => {
              pasa.groupLeave(_dir[position].id)
